...
onMounted(async () => {
  try {
    const response = await $fetch<PokemonSummary[]>('/api/pokemon')
    pokemonList.value = response
  } catch (err: any) {
    error.value = err.message || 'Failed to load Pokémon.'
  } finally {
    loading.value = false
  }
})
...
const filteredList = computed(() => {
  ...
  return pokemonList.value.filter(p => p.name.toLowerCase().includes(term))
})
</script>
...
<template>
  <div class="container">
    ...
    <p v-if="!loading && !filteredList.length" class="empty">
      No Pokémon found.
    </p>
  </div>
</template>